...
基于 DESIGN.md 的完整设计令牌体系，从颜色、排版、圆角、间距到组件变体全面标准化，确保视觉一致性和开发效率。核心设计语言特征：

- 暖调奶油画布 + 深色墨水文字，营造编辑级阅读体感
- 衬线体标题用于信息分层（Copernicus / Tiempos Headline 风格）
- 零圆角/小圆角的"粗野主义"按钮与输入框，传递专业可靠感
- 四级病害严重度色标（轻度绿 → 中度琥珀 → 高度警告 → 严重铁锈红）
...
graph TB
    ...
    subgraph 服务层["外部服务"]
        C1[后端 REST API<br/>hellodpl.vip.cpolar.cn]
        C2[扣子 Coze 大模型<br/>api.coze.cn/v3/chat]
        C3[高德地图 API<br/>restapi.amap.com]
    end
...
- **用户信息区**：头像（网络图片）+ 昵称 + 角色标签
- **数据概览**：大号数字展示诊断总数和独特病害种类数
- **快捷入口**：诊断记录入口
- **退出登录**按钮
...
│   └── route.dart                          # 命名路由配置
└── utils/
    ├── api_config.dart                     # API 密钥与终端配置（gitignore）
    ├── api_config_example.dart             # 配置模板
    ├── http_util.dart                      # Dio HTTP 封装（GET / POST / 流式）
...
- Android SDK（API 21+）或 iOS 开发环境（Xcode 15+）
...
# 配置 API 密钥
cp lib/utils/api_config_example.dart lib/utils/api_config.dart
# 编辑 api_config.dart 填入后端地址、Coze 令牌、高德 API Key
